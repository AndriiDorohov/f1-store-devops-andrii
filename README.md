...
For portfolio and documentation, the following screenshots are used:

1. **API Connection Tester UI**

   Browser window showing `http://localhost:8080/` (or `http://<EC2_PUBLIC_IP>:8080/`) with both RDS and Redis tests.

   ![API Connection Tester UI](screenshots/api-connection-tester.png)

2. **AWS S3 Bucket**

   S3 console page showing the bucket `f1-store-frontend-andrii-eu-north-1` with `index.html`.

   ![AWS S3 bucket](screenshots/aws-s3-bucket.png)

3. **CloudFront Distribution**

   CloudFront console showing the distribution `f1-store-frontend-andrii` with domain and origin configuration.

   ![AWS CloudFront distribution](screenshots/aws-cloudfront-distribution.png)

4. **EC2 + Docker**

   Terminal on the EC2 instance with `docker ps` output listing:
   `f1_frontend`, `f1_backend_rds`, `f1_backend_redis`, `f1_postgres`, `f1_redis`.

   ![EC2 instance with Docker containers](screenshots/aws-ec2-docker-ps.png)

5. **GitHub Actions**

   GitHub Actions view for the workflow  
   **“Build, push Docker images to ECR and deploy to EC2”** with a successful run.

   ![GitHub Actions CI/CD workflow](screenshots/github-actions-success.png)
...
1. **Containerization of multi-service apps**
   - Structured Dockerfiles for multiple services (two Django backends + Nginx frontend).
   - Isolated configuration and secrets via `.env` and `.env.example`.
